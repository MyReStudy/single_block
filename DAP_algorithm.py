...
if __name__ == '__main__':
    ...
    for ii in range(1,21):
        ...
        for jj in range(1,101):
            ...
            picking = DAP(distanceMatrixPath, warehouseInformation)
            _, l, path = picking.formingPath()
            time_end = time.perf_counter()
            time_sum = time_end - time_start
            time_tmp.append(time_sum)
            result_total.append(l)
        total_time.append(np.mean(time_tmp))

    # 创建一个DataFrame，列表作为列
    df_time = pd.DataFrame({'Column1': total_time})
    # 保存为Excel文件
    df_time.to_excel('DAP-time.xlsx', index=False)  # index=False表示不保存行索引

    df_result = pd.DataFrame({'Column2': result_total})
    df_result.to_excel('DAP-result.xlsx', index=False)  # index=False表示不保存行索引
